...
在无误码的情况下，信道利用率 $eta = frac(w, 2 + 2 a)$。本程序中，$w = 63$，$a = frac(270 m s, 263 m s)$，所以 $eta$ 已经达到 100% 的效率。在有误码的情况下，以 $b e r = 10 ^ (-4)$ 为例，成功发出一帧的概率为 $P = (1 - 10 ^ (-4)) ^ (8 times 263) = 81.025%$。所以 $eta = P eta = 81.025%$。
...
#figure(
  align(center)[#table(
    columns: (5.66%, 26.42%, 7.55%, 15.09%, 15.09%, 15.09%, 15.09%),
    align: (auto,auto,auto,auto,auto,auto,auto,),
    table.header([序号], [说明], [运行时间(秒)], [GBN A
      接收方信道利用率], [GBN B 接收方信道利用率], [SR A
      接收方信道利用率], [SR B 接收方信道利用率],),
    table.hline(),
    [1], [无误码信道数据传输], [1200], [51.59%], [96.97%], [54.23%], [96.97%],
    [2], [站点 A 分组层平缓方式发出数据，站点 B 周期性交替“发送100
    秒，慢发 100 秒”], [1200], [44.61%], [82.62%], [50.38%], [92.21%],
    [3], [无误码信道，站点 A 和站点 B
    的分组层都洪水式产生分组], [1200], [96.97%], [96.97%], [96.96%], [96.96%],
    [4], [站点 A/B 的分组层都洪水式产生分组], [1200], [84.87%], [83.05%], [93.36%], [92.98%],
    [5], [站点 A/B
    的分组层都洪水式产生分组，线路误码率设为$10^(- 4)$], [1200], [24.09%], [22.12%], [61.05%], [60.41%],
  )]
  , kind: table
  )
...
+ #strong[站点 A 平缓发出数据，站点 B
  周期性交替发送与慢发];：
  - 利用率相比洪水式发送有所下降。这是因为站点B发送速率的周期性变化使得难以进行捎带确认。
+ #strong[无误码信道，站点A和B都洪水式产生分组];：
  - 接收方信道利用率非常高，几乎达到了极限，显示在高数据负载下协议能有效处理传入数据。
+ #strong[站点A/B的分组层都洪水式产生分组，线路误码率设为$10^(- 4)$];：
  - 误码率的增加显著降低了信道利用率。这表明错误恢复机制在处理高误码环境时效率不高，可能导致大量重传和错误处理开销。同时我们可以看到，在高误码率情况下，SR协议的性能相对更好。
...
+ #strong[监控和调整反馈策略];：
  - 实施更有效的反馈策略，优化ACK包的发送频率和内容，减少确认带来的额外负载。
...
#quote(block: true)[
本次实验中 CRC32 校验和的计算直接调用了一个简单的库函数，8.8
节中的库函数 crc32()是从 RFC1662 中复制并修改而来。在 PPP 相关协议文本
RFC1662.TXT（以另外单独一个文件提供）中含有计算 CRC-32 和 CRC-16
的源代码，浏览这些源代码。教材中给出了手工计算 CRC 校验和的方法，通过二
进制“模2”除法求余数。这些源代码中却采用了以字节值查表并叠加的方案，看起来计算速度很快。你能分析出这些算法与我们课后习题中手工进行二进制“模
2”除法求余数的算法是等效的吗？算法中设置的查表数组
crc\_table\[256\]数组是怎样构造出来的？你能否写一段 C 语言程序，按照模 2
除法的规则， 用你的程序生成速查表 crc\_table 的 256
个数字？或者，给出一个例子手工验证表中的某项是怎样得来的。在 x86
系列计算机上为某一帧计算 32 位 CRC32 校验和，比较一下：为相同帧计算 16
位 CRC16 校 验和，所花费的 CPU 时间会多一倍吗？本次实验课提供的 crc32
函数只有两个参数，分别为缓冲区首地
址和缓冲区长度，这似乎足够了，可是，RFC1662 给出的源代码样例中的函数
pppfcs32(fcs, cp, len) 需要三个参数，为什么要这样设计？
]
...
#quote(block: true)[
8.10节提出的协议软件的跟踪功能有什么意义？你的程序实现这样的功能了吗？程序库中获取时间坐标的函数 get\_ms()不是 C语言标准库中的函数，你能自己实现一个这样的函数吗？ 在“C语言程序设计”课程中学习过 printf 函数，printf
风格的函数特点是参数数目不确定，类似的标准库函数还有
fprintf，sprintf，scanf
等。为了便于调试程序，程序库中提供了日志文件和屏幕窗 口同步输出的
lprintf 函数(参见 8.4)，这些函数都不是标准的 C 语言库函数，调用风格与
printf 类似。可以参考给出的源代码，看看 printf 风格的函数是怎样实现的。
8.10 节中给出了两个函数 start\_timer()和
start\_ack\_timer()，它们都是定时器函数，两个函数启动定时器的时机不同，而且在定时器到时之前重新调用函数对原残留时间的处理方式也不同，为什么要这样设计？
]
...
=== 实现跟踪功能的考虑
<实现跟踪功能的考虑>
在协议开发中实现跟踪功能，通常包括以下几个方面： -
#strong[日志记录];：记录每个重要事件的发生时间和相关数据，这有助于事后分析。
\- #strong[错误跟踪];：特别标记错误事件和异常条件，以便快速定位问题。 -
#strong[性能度量];：记录关键操作的执行时间和其他性能指标，以评估协议的效率。

=== `get_ms()` 函数的实现
<get_ms-函数的实现>

在C语言标准库中没有直接提供获取毫秒级时间戳的函数。但可以通过以下几种方法实现：

#strong[在Linux平台上];，可以使用`gettimeofday()`函数来实现，代码示例如下：
...
#strong[在Windows平台上];，可以使用`GetTickCount()`函数：
...
=== 定时器函数设计的考虑
<定时器函数设计的考虑>
`start_timer()` 和 `start_ack_timer()`
的不同设计反映了它们在协议中的不同用途：`start_timer()` 被设计为在物理层发送队列清空之后开始计时。这确保了计时从所有相关数据发送完成后开始，从而更准确地反映超时前等待ACK的时间。`start_ack_timer()` 则立即开始计时，需要快速响应，不依赖于数据发送的完成。
...
#quote(block: true)[
验证所完成的程序能否在各种情况下都能够正确工作，是软件测试环节的主要目的。表 3中列出了七种测试方案，设计这么多种测试方案的目的是什么？分析每种测试方案，每种方案主要是为了瞄准你的协议软件中可能出问题的哪些环节？或者说，你的协议软件存在什么问题时，测试会失败？你觉得还存在有哪些问题是这些测试尚未覆盖的？这些测试方案和验证协议正确性的手段由指导教师给出，如果是由你自己独立完成整个协议的设计和测试，你会采用哪些手段来验证你的程序能正确工作？针对本次实验的具体问题，你能不能提出一种更高效的软件测试方案？本次实验所提供的程序库还有哪些不足，怎样才能对协议开发提供更方便的支撑，这关系到在整体软件开发过程中的不同模块间的功能划分问题，给出你的建议。
]

在软件测试阶段，尤其是针对通信协议软件的测试，设计多种测试方案的目的是为了确保协议在不同的网络条件和应用场景下都能够正确、有效地工作。这些测试方案通常设计来模拟现实世界中可能遇到的各种极端和常规条件，以便揭示潜在的缺陷和问题。下面对表3中列出的测试方案进行分析，看看它们各自针对协议软件可能出现的哪些问题：

=== 测试方案分析
<测试方案分析>
+ #strong[无误码信道数据传输];：
  - #strong[目的];：验证在理想条件下，协议是否能够无误地传输数据。
  - #strong[可能发现的问题];：基本功能失效，如数据不完整、顺序错误等。
+ #strong[站点A分组层平缓方式发出数据];：
  - #strong[目的];：检测协议在正常速率下的数据处理能力和流量控制机制。
  - #strong[可能发现的问题];：发送方的缓冲管理、流量控制不当导致数据丢失或延迟。
+ #strong[站点B周期性交替“发送100秒，慢发100秒”];：
  - #strong[目的];：测试协议在变化的网络带宽下的适应性和鲁棒性。
  - #strong[可能发现的问题];：动态网络条件下流量控制和拥塞控制机制不足。
+ #strong[无误码信道且站点A和站点B的分组层都洪水式产生分组];：
  - #strong[目的];：检验在高负载下协议的性能和稳定性。
  - #strong[可能发现的问题];：在极端负载下可能出现的性能瓶颈、处理延迟和缓冲区溢出。
+ #strong[站点A/B的分组层都洪水式产生分组];：
  - #strong[目的];：模拟高负载同时伴随着轻微误码的网络环境，测试误码恢复机制。
  - #strong[可能发现的问题];：错误恢复策略不充分，如自动重传请求（ARQ）处理不当。
+ #strong[站点A/B的分组层都洪水式产生分组，线路误码率设为$10^(- 4)$];：
  - #strong[目的];：在更高误码率下测试协议的错误检测和纠正能力。
  - #strong[可能发现的问题];：高误码环境下的数据完整性问题，CRC或校验和算法可能不够强大。

=== 未覆盖的问题
<未覆盖的问题>
- #strong[网络分段和重组];：测试方案可能没有考虑数据包分段和重组的效率和正确性。
- #strong[多种网络拓扑];：协议在不同的网络拓扑结构中的表现可能未被充分测试，如多跳网络环境。
- #strong[安全性测试];：数据传输的加密和认证机制未被提及。

=== 独立完成协议设计和测试的方法
<独立完成协议设计和测试的方法>
如果由我独立完成整个协议的设计和测试，我会采用以下手段：

- #strong[单元测试];：针对协议的每个独立模块进行详尽的测试。
- #strong[集成测试];：测试模块间的接口和交互。
- #strong[模拟测试];：使用网络模拟器模拟不同的网络条件。
- #strong[性能测试];：评估在不同负载和网络条件下的协议性能。
- #strong[安全性测试];：确保数据传输的安全性和数据完整性。
...
- #strong[增强调试和分析工具];：集成更先进的调试和性能分析工具。使用类似 WireShark 的网络分析工具，可以更好地监控和分析数据包的传输。
...
#quote(block: true)[
如果现实中有两个相距 5000 公里的站点要利用你所设计的协议通过卫星信道进行通信，还有哪些问题需要解决？实验协议离实用还有哪些差距？你觉得还需要增加哪方面的功能？从因特网或其他资料查阅
LAPB 相关的协议介绍和对该协议的评论，用成熟的 CCITT 链路层协议标准对比你所实现的实验性协议，实验性协议的设计还遗漏了哪些重要问题？
]
...
=== 结论
<结论>
综上所述，虽然实验性协议提供了基础的链路层控制，但要达到LAPB那样成熟、稳定且能有效应对卫星通信特有挑战的水平，还需要进一步的研究和开发。这包括对协议参数的动态调整、错误控制的增强以及流量控制策略的优化。
...
预计的完成时间为4~6个小时，这和我的完成时间基本一致，这包括编程、调试和多次测试来验证程序的正确性和性能。

==== 协议方面问题
<协议方面问题>

+ #strong[协议死锁];：在特定条件下，发送和接收过程陷入死锁状态。这是由于确认消息处理不当导致的（序号处理错误）。对协议状态机进行了重构，解决了死锁问题。

==== 综合提升
<综合提升>
- #strong[C语言技能];：对C语言的掌握更加熟练，特别是在指针操作、内存管理和模块化编程方面。
- #strong[协议软件开发];：对网络协议的理解更深入，能够设计和实现复杂的通信协议。
- #strong[理论知识];：通过实践加深了对数据链路层滑动窗口协议的理解。
- #strong[软件工程];：学习了软件开发的整个生命周期，包括需求分析、设计、编程、测试和维护等。
